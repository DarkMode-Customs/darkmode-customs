...
function initAdminWindowLinks() {
  const adminLinks = document.querySelectorAll("[data-admin-window]");
  ...
  adminLinks.forEach((link) => {
    link.addEventListener("click", (event) => {
      ...
      const adminUrl = link.getAttribute("href") || "admin.html";
      const adminWindow = window.open(
        adminUrl,
        "DMCAdminPanel",
        "popup=yes,width=1180,height=900,left=120,top=80,noopener,noreferrer"
      );
